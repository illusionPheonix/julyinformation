        ...
        <el-card class="box-card">
            ...
            <div class="text item">
                ...
                >
                    <el-form :model="editFormData" size="small" ref="editFormData" :rules="rules">
                        ...
                        <el-form-item label="课程选择" prop="courseData">
                            <el-select v-model="editFormData.courseData" multiple placeholder="请选择课程">
                                <el-option
                        ...
                        <el-form-item label="初始积分" prop="score">
                            <el-input type="text" v-model="editFormData.score" auto-complete="on"></el-input>
                        </el-form-item>
                    </el-form>
                    <!-- 尾部 -->
                ...
                <div class="filterBox">
                    <!-- 关键字 -->
                    关键字：
                    <el-input type="text" v-model="keyword" auto-complete="off"  size="small" placeholder="姓名"></el-input>
...
<script>
// 引入验证用户名函数
import { nameReg,telReg,feesReg } from "@/utils/validator/validator.js";
// 引入moment
    ...
    methods:{
     // 分页获取列表函数
        getListByPages(){
            ...
            // 向后端发送请求
            this.request.get('/student/studentmanage',params)
            .then(res=>{
                // 接收后端发送回来的数据
                let {total,data}=res;
                // 赋值给页面总条数
        ...
        handleEdit(id) {
            ...
            // 发送请求
            this.request.get('/student/editstudent',{id})
            .then(res=>{
                console.log(res);
                ...
                this.editFormData.score=res[0].score
            })
            .catch(err=>{
                console.log(err);
            })
        ...
        saveEdit(){
            ...
            let params = {
                ...
                score:this.editFormData.score
            };    
             // 判断：表单通过验证后发送请求到后台
            this.$refs.editFormData.validate(valid=>{
                if(valid){
                    // 发送请求
                     this.request.post('/student/savestudent',params)
                    .then(res=>{
                        // 接收后端传回来的数据
                        let { code,msg } = res;
                        if(code === 0){
        ...
        handleDelete(id) {
            ...
            this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
                ...
                // 发送请求到后端
                this.request.get('/student/studentdel',{id})
                .then(res=>{
                    // 接收后端发送回来的数据
                    let {code,msg}=res;
                    if(code===0){
        ...
        batchDel(){
            ...
            this.$confirm('是否批量删除?', '提示', {
                ...
                type: 'warning'
                    ...
                    // 向后端发送参数
                    this.request.get('/student/batchdel',params)
                    .then(res=>{
                        // 接收传回来数据
                        let {code,msg}=res;
                        if(code===0){
